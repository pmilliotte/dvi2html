...
export default class HTMLMachine extends Machine {
  ...
  preamble(
    ...
    let tfm_conv =
      ((25400000.0 / numerator) * (denominator / 473628672)) / 16.0;
  ...
  putText(text: Buffer): number {
    ...
    var dviUnitsPerFontUnit =
      ((this.font.metrics.designSize / 1048576.0) * 65536) / 1048576;

    var top =
      (this.position.v - textHeight * dviUnitsPerFontUnit) *
    ...
    let fontsize =
      ((this.font.metrics.designSize / 1048576.0) * this.font.scaleFactor) /
    ...
    if (this.svgDepth == 0) {
      this.output.write(
        `<span style="line-height: 0; color: ${this.color}; font-family: ${
          ...
          top - height
        }pt; left: ${left}pt; overflow: visible;"><span style="margin-top: -${fontsize}pt; line-height: ${0}pt; height: ${fontsize}pt; display: inline-block; vertical-align: baseline; ">${htmlText}</span><span style="display: inline-block; vertical-align: ${height}pt; height: ${0}pt; line-height: 0;"></span></span>`
      );
    } else {
      let bottom = this.position.v * this.pointsPerDviUnit;
    ...
    return (
      (textWidth * dviUnitsPerFontUnit * this.font.scaleFactor) /
      this.font.designSize
    );
  }
}
